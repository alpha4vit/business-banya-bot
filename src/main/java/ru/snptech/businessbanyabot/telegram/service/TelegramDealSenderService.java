package ru.snptech.businessbanyabot.telegram.service;
...
import static ru.snptech.businessbanyabot.telegram.TelegramUtils.escapeMarkdownV2;
...
public class TelegramDealSenderService {
    ...
    private List<TelegramUser> getPartnerAndAssistantsTelegramAccount(TelegramUser currentUser) {
        return Optional.ofNullable(currentUser.getPartnerAccount())
                .map(partner -> userRepository.findAllByChatIdOrPartnerAccount_ChatId(partner.getChatId(), partner.getChatId()))
                .orElseGet(
                        () -> userRepository.findAllByChatIdOrPartnerAccount_ChatId(currentUser.getChatId(), currentUser.getChatId())
                );
    }
